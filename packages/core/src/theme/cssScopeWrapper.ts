    ...
    (match, selectors, properties) => {
      ...
      const wrappedSelectors = selectors
        ...
        .map((selector: string) => {
          ...
          // 处理 .dark 暗色模式选择器
          // .dark 类在 html 元素上，在 #output 外面
          // 所以 .dark h1 应该变成 .dark #output h1，而不是 #output .dark h1
          // 使用正则匹配以处理可能的注释或空白（[\s\S] 匹配包括换行在内的任意字符）
          const darkMatch = trimmed.match(/^([\s\S]*)\.dark\s+(.+)$/)
          if (darkMatch) {
            const prefix = darkMatch[1] // 可能的注释或空白
            const restSelector = darkMatch[2]
            return `${prefix}.dark ${scope} ${restSelector}`
          }
