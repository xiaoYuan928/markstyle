...
| 模块 | 状态 | 说明 |
|-----|------|------|
| Next.js 框架 | ✅ 已完成 | apps/web 基础架构 |
| Markdown 编辑器 | ✅ 已完成 | CodeMirror 集成 |
| 实时预览 | ✅ 已完成 | 左写右看 |
| 主题系统 | ✅ 已完成 | 20+ 主题（简洁/墨韵/霓虹/手账/极简黑/渐变/多彩/科技蓝/杂志/双色/玻璃/方格本/报纸/信笺/卡片/涂鸦等） |
| 一键复制到公众号 | ✅ 已完成 | CSS 内联 + 微信兼容，Header 已接入 |
| 富文本粘贴转换 | ✅ 已完成 | turndown 转换飞书/Notion HTML |
| AI 生成封面 | ✅ 已完成 | AI 描述 + Z-Image-Turbo 出图，支持 5 种尺寸 |
| 额度/账号策略 | ✅ 已上线 | 免登录可用；未登录每日 3 次免费生成；首次注册赠 20 积分；登录可看历史与云端文档 |
| 落地页/营销 | ⏳ 进行中 | 需更新卖点、主题预览展示、免费策略说明 |
...
- 简洁、墨韵（国风）、霓虹（赛博）、手账（手绘）、极简黑（商务）、渐变、多彩、科技蓝
- 杂志（纸感）、双色（日系极简）、玻璃（拟态）、方格本（网格手账）、报纸（衬线 dropcap）、信笺（邮戳）、卡片（百科信息框）、涂鸦（手绘符号）

**后续迭代**：继续补充小众风格（如复古打字机、玻璃变体、图文卡片混排），并生成精选预览缩略图
...
- 免登录：可直接使用编辑/预览/复制，且每日 3 次免费 AI 封面生成额度。
- 首次注册：赠送 20 积分，可额外生成 20 张封面。
- 登录权益：查看封面生成历史、云端保存文档、额度统计。
- 超额策略：后续可配置套餐/邀请赠送（待定）。

## 四、开发阶段规划（已完成为主，新增落地页任务）
...
|-------|------|---------|--------|
| Phase 1 | 集成核心模块 | 已完成 | - |
| Phase 2 | 一键复制到公众号 | 已完成 | - |
| Phase 3 | 主题系统扩展（20+ 主题） | 已完成 | - |
| Phase 4 | 富文本粘贴转换 | 已完成 | - |
| Phase 5 | UI 优化与品牌 | 已完成 | - |
| Phase 6 | AI 生成封面 | 已完成 | - |
| Phase 7 | 落地页更新（卖点/预览/免费策略） | 1-2 天 | P0 |

**当前剩余工作：落地页更新、后续增量主题与缩略图**
...
- [x] 20+ 主题可用且在公众号显示正确
- [x] 主题切换实时预览
- [x] 一键复制到公众号，样式在微信正确显示
- [x] 支持从飞书/Notion 粘贴内容自动转 Markdown
- [x] 粘贴后 500ms 内完成渲染
- [x] AI 生成封面（含描述生成、尺寸选择、历史记录）
- [ ] 落地页展示最新卖点（主题数量、AI 封面、免登录/登录赠额）
- [ ] 主题预览缩略图/示例图展示，提升转化
...
- 主题：`packages/shared/src/configs/theme-css/index.ts`、`packages/shared/src/configs/theme.ts`
- 复制：`apps/web/src/utils/clipboard.ts`、`apps/web/src/components/editor/EditorHeader.tsx`
- 粘贴：`apps/web/src/utils/htmlToMarkdown.ts`、`apps/web/src/components/editor/CodeMirrorEditor.tsx`
- 主题选择：`apps/web/src/components/editor/SettingsPanel.tsx`
- AI 封面：`apps/web/src/components/editor/CoverGeneratorDialog.tsx`、`CoverHistoryDialog.tsx`
- 存储：`apps/web/src/stores/post.ts`（IndexedDB + 云端）
- 落地页（待更新）：`apps/web/src/components/landing/*`
...
| 日期 | 更新内容 |
|------|---------|
| 2025-01-XX | 同步免费策略、20+ 主题、AI 封面完成，新增落地页待办 |
| 2025-12-18 | 创建开发计划文档 |
